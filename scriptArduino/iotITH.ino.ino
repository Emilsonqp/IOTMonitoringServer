...
const char ssid[] = "emilsonqphuawei"; // TODO cambiar por el nombre de la red WiFi
// Contraseña de la red WiFi
const char pass[] = "emilsondqp503"; // TODO cambiar por la contraseña de la red WiFi
...
const char MQTT_HOST[] = "18.205.188.252"; // TODO Reemplace ip.maquina.mqtt por la IP del bróker MQTT que usted desplegó. Ej: 192.168.0.1
...
float humi;
// Valor del indice de temperatura-humedad
float ith;
 ...
 */
void sendSensorData(float temperatura, float humedad, float ith) {
  String data = "{";
  data += "\"temperatura\": "+ String(temperatura, 1) +", ";
  data += "\"humedad\": "+ String(humedad, 1) +", ";
  data += "\"ith\": "+ String(ith, 1);
  data += "}";
...
/**
 * Calcula el ITH, lo imprime en consola y la devuelve.
 */
float readIth() {
  // Se calcula el Ith
  float i = (0.8)*dht.readTemperature() + (dht.readHumidity()/100)*(dht.readTemperature()-14.3) + 46.4;
...
String checkAlert() {
  ...
  if (alert.length() != 0) {
    ...
    //Serial.print(alertTime);
    message = alert;
    result = (alertTime > millis()) ? 0 : millis()-alertTime;
    if (result > 0 ) {
      alert = "";
      alertTime = millis();
      Serial.print("LIMPIADO!!!");
     }
  }
  return message;
}
...
void receivedCallback(char* topic, byte* payload, unsigned int length) {
  alertTime = millis() + (ALERT_DURATION * 1000);
